  ...
  **/
  ...
  Object.keys(ctx.models).forEach(modelName => {
    if (ctx.models[modelName].sharedClass.ctor.settings.sdk &&
      ...
      let resultSet = `${modelName}ResultSet`;
      schema.push(
        /**
        * SDK MODELS
        */
        {
          ...
          output: '/models/' + capitalize(resultSet) + '.ts',
          params: {
    ...
    if (relations.length > 0) {
      relations.forEach((relationName, i) => {
        ...
        if (!loaded[targetClass] && targetClass !== modelName) {
          ...
          output.push(`  ${targetClass}ResultSet`);
        }
      });
    }
  ...
  function buildServiceImports(model, loadAccessToken, isIo) {
    ...
    let imports = [
      ...
      { module: `${modelName}ResultSet`, from: `../../models/${modelName}ResultSet`},
    ];
    if (isIo === 'enabled') {
    ...
    getModelRelations(model).forEach((relationName, i) => {
      ...
      if (
        ...
        if (!loaded[through] && targetClass !== modelName) {
          loaded[through] = true;
          imports.push({ module: `${through}ResultSet`, from: `../../models/${through}ResultSet` });
        }
      }
      ...
      if (!loaded[targetClass] && targetClass !== modelName) {
        loaded[targetClass] = true;
        imports.push({ module: `${targetClass}ResultSet`, from: `../../models/${targetClass}ResultSet` });
      }
    });
...
// NEW TODO let dependencies = ['@Inject(HttpClient) protected http: HttpClient'];
    if (isIo === 'enabled') {
...
// NEW TODO      { module: 'HttpClient', from: '@angular/common/http'},
      { module: 'NgModule, ModuleWithProviders', from: '@angular/core'},
